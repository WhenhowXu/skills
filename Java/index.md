...
- 网络编程基础
  - **互联网**：是网络的网络（internet），即把很多计算机网络连接起来，形成一个全球统一的互联网
  - **IP 地址**：一个 IP 地址用于唯一标识一个网络接口（Network Interface）
    - IPv4: 32 位地址，目前已耗尽
    - IPv6: 64 位地址
    - 特殊的 IP 地址：本机地址，总是`127.0.0.1`
  - **网关**: 作用就是连接多个网络，负责把来自一个网络的数据包发到另一个网络，这个过程叫`路由`
  - **域名**: 直接记忆 IP 地址非常困难，所以我们通常使用域名访问某个特定的服务
    - 域名解析服务器 DNS 负责把域名翻译成对应的 IP，客户端再根据 IP 地址访问服务器
    - 特殊的本机域名`localhost`， 对应`127.0.0.1`
  - **网络模型**：
    - 应用层，提供应用程序之间的通信；
    - 表示层：处理数据格式，加解密等等；
    - 会话层：负责建立和维护会话；
  ...
  - **协议**
    - `IP协议`：分组交换，它不保证可靠传输
    - `TCP协议`：
      - 传输控制协议，它是面向连接的协议，支持可靠传输和双向通信
      - TCP 协议是建立在 IP 协议之上的
      - TCP 协议允许双向通信
      - TCP 协议也是应用最广泛的协议，许多高级协议都是建立在 TCP 协议之上的，例如 HTTP、SMTP
    - `UDP协议`（User Datagram Protocol): 是一种数据报文协议，它是无连接协议，不保证可靠传输；传输效率比 TCP 高
